...
class AudioClassifier (nn.Module):
    ...
    def __init__(self):
        ...
        conv1 = nn.Conv2d(1, 200, kernel_size=(3, 3), stride=(2, 2), padding=(2, 2))
        relu1 = nn.ReLU()
        pool1 = nn.MaxPool2d(2)
        bn1 = nn.BatchNorm2d(200)
        init.kaiming_normal_(conv1.weight, a=0.1)
        conv1.bias.data.zero_()
        conv_layers += [conv1, relu1, pool1, bn1]

        for i in range(4):
          conv = nn.Conv2d(200, 200, kernel_size=(3, 3), stride=(2, 2), padding=(1, 1))
          relu = nn.ReLU()
          bn = nn.BatchNorm2d(200)
        ...
        self.lin = nn.Linear(in_features=200, out_features=2)
...
myModel.load_state_dict(torch.load('model_weights_epoch11.pth'))
